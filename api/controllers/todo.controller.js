...
exports.createTodo = async (req, res) => {
  try {
    ...
    // INSERT INTO todos (actions, status) VALUES ('A new todo', 'Saved in db', 'active')
    const newTodo = await Todo.create({
      content: content // dbColumn: valueToInsert
    });
...
exports.updateTodoPatch = async (req, res) => {
  try {
    ...
    const data = filterObj(req.body, 'content'); // { aciotns } | { action, status }
    ...
    await todo.update({ ...data }); // .update({ action, status })
